...
// Tracks active toast dedup keys to prevent stacking
const _activeToastKeys = new Set();

// Debounce timer specifically for validation (input-spam) toasts
let _validationToastTimer = null;
let _validationToastEl = null;

function showValidationToast(title, message) {
  // Clear any pending timer so rapid keypresses don't stack
  if (_validationToastTimer) {
    clearTimeout(_validationToastTimer);
    _validationToastTimer = null;
  }

  // If a validation toast is already visible, just refresh its timer — no new toast
  ...
  // Remove any lingering validation toast before creating a new one
  ...
  // Append directly to body so it is not affected by container stacking
...
function showToast(type, title, message, actionLabel, actionCallback, customClass) {
  ...
  // Build a dedup key from type + title to prevent stacking identical toasts
  const dedupKey = `${type}::${title}`;

  // For mismatch toasts use customClass as key so the existing removeExistingMismatchToast() flow still governs them
  ...
  // Allow manual close to also clear the dedup key
...
function getCart() {
  try {
    ...
    // Normalize each item — handles old/malformed saves
...
function renderCart() {
  ...
  // ✅ Guard all fields that could be undefined
