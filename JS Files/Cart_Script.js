...
// tracks the last known mismatch type to avoid repeat toasts
let lastShippingCurrencyMismatch = null;

// check if the selected currency doesn't match the shipping destination
function getCurrencyShippingMismatch() {
  if (!destinationType) return null;

  // international shipping should not use php
  ...
  // domestic shipping should use php only
...
// remove any existing shipping mismatch toast from the container
...
// show a mismatch toast only when the mismatch type changes or is forced
function validateCurrencyShippingMatch({ forceToast = false } = {}) {
  ...
  // skip if nothing has changed and not forced
...
// hide the cart icon when viewing the cart page itself
...
function showToast(type, title, message, actionLabel, actionCallback, customClass) {
  ...
  // toasts stay visible for 5 seconds
